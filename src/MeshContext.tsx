...
function getSignalingUrl(): string {
  if (Platform.OS === "ios" || Platform.OS === "android") {
    ...
    return ""; // no IP yet — user must enter in Settings
  }
  try {
...
// ─── Pure-JS AES-128-CTR + HMAC-SHA256 for native (Hermes has no SubtleCrypto) ─
// Used only when Platform.OS !== "web". Provides authenticated encryption
// comparable to AES-GCM: CTR mode for confidentiality + HMAC for integrity.
// Key derivation: SHA-256(sharedSecret) → first 16 bytes AES key, last 16 bytes MAC key.

function _aesBlock(key: Uint8Array, block: Uint8Array): Uint8Array {
  // AES-128 single block encryption (Rijndael) — pure JS
  ...
  // Key expansion
  const Nk = 4, Nr = 10;
  const w = new Uint8Array(44 * 4);
  w.set(key.slice(0, 16));
  const rcon = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36];
  for (let i = Nk; i < 44; i++) {
    let t0=w[(i-1)*4],t1=w[(i-1)*4+1],t2=w[(i-1)*4+2],t3=w[(i-1)*4+3];
    if (i % Nk === 0) {
      const tmp=t0; t0=sbox[t1]^rcon[i/Nk-1]; t1=sbox[t2]; t2=sbox[t3]; t3=sbox[tmp];
    }
  ...
  for (let i=0;i<16;i++) s[i]=block[i]^w[i];
  for (let r=1;r<=Nr;r++) {
    // SubBytes
...
// Derive a 32-byte key from a shared secret string using SHA-256
function _deriveNativeKey(secret: string): Uint8Array {
  const data = new TextEncoder().encode(secret + ":hexmesh-e2e-v1");
  return base64ToUint8Array(sha256Sync(data).match(/.{2}/g)!.map(h => parseInt(h,16)).reduce(
    (arr, b) => { arr.push(b); return arr; }, [] as number[]
  ).reduce((s, b, i) => s + String.fromCharCode(b), "").split("").reduce((a,c) => a + c, ""));
}

// AES-128-CTR encrypt/decrypt (symmetric — same function both ways)
function _aesCtr(key16: Uint8Array, nonce12: Uint8Array, data: Uint8Array): Uint8Array {
  const out = new Uint8Array(data.length);
  const counter = new Uint8Array(16);
  counter.set(nonce12, 0); // bytes 0-11 = nonce, 12-15 = counter (big-endian)
  for (let i = 0; i < data.length; i += 16) {
    const ks = _aesBlock(key16, counter);
    for (let j = 0; j < 16 && i + j < data.length; j++) out[i + j] = data[i + j] ^ ks[j];
    // Increment counter (bytes 12-15)
    for (let k = 15; k >= 12; k--) { counter[k] = (counter[k] + 1) & 0xff; if (counter[k]) break; }
  }
  return out;
}

// HMAC-SHA256 using pure-JS sha256Sync
function _hmac(key: Uint8Array, data: Uint8Array): Uint8Array {
  const blockSize = 64;
  let k = key.length > blockSize ? base64ToUint8Array(sha256Sync(key)) : key;
  ...
  const innerHash = base64ToUint8Array(sha256Sync(inner));
  const outer = new Uint8Array(blockSize + 32);
  outer.set(opad); outer.set(innerHash, blockSize);
  return base64ToUint8Array(sha256Sync(outer));
}

// Native E2E: encrypt plaintext with AES-128-CTR + HMAC-SHA256
// Output format: base64(nonce[12] + hmac[32] + ciphertext)
function _nativeEncrypt(keyMaterial: string, plaintext: string): string {
  const keyBytes = base64ToUint8Array(sha256Sync(new TextEncoder().encode(keyMaterial + ":enc")));
  const macBytes = base64ToUint8Array(sha256Sync(new TextEncoder().encode(keyMaterial + ":mac")));
  const key16 = keyBytes.slice(0, 16);
  const nonce = new Uint8Array(12);
  // Simple nonce from timestamp + random (no crypto.getRandomValues on old Hermes)
  const ts = Date.now();
  for (let i = 0; i < 8; i++) nonce[i] = (ts / Math.pow(256, i)) & 0xff;
  for (let i = 8; i < 12; i++) nonce[i] = Math.floor(Math.random() * 256);
  const pt = new TextEncoder().encode(plaintext);
  const ct = _aesCtr(key16, nonce, pt);
  const mac = _hmac(macBytes.slice(0, 32), ct).slice(0, 16); // 16-byte truncated HMAC
  const out = new Uint8Array(12 + 16 + ct.length);
  out.set(nonce, 0); out.set(mac, 12); out.set(ct, 28);
  return uint8ArrayToBase64(out);
}

function _nativeDecrypt(keyMaterial: string, b64: string): string | null {
  try {
    ...
    const mac   = data.slice(12, 28);
    const ct    = data.slice(28);
    const expectedMac = _hmac(macBytes.slice(0, 32), ct).slice(0, 16);
    // Verify HMAC
    let diff = 0;
    for (let i = 0; i < 16; i++) diff |= mac[i] ^ expectedMac[i];
    if (diff !== 0) { console.warn("[E2E] HMAC mismatch — tampered message"); return null; }
    const pt = _aesCtr(key16, nonce, ct);
    return new TextDecoder().decode(pt);
  } catch { return null; }
}

// Native shared key store (keyed by peerId, value is ECDH nodeId concat)
const _nativeSharedSecrets = new Map<string, string>(); // peerId → shared secret string
...
// ─── Packet signing — ECDSA P-256 ─────────────────────────────────────────────
// Each node generates a persistent signing keypair on first run.
// Private key signs outgoing control packets. Peers verify using the public key
// announced in ANNOUNCE/REGISTERED. This gives real sender authentication:
// no shared secret, each node can only forge packets signed by its own key.
//
// Key storage: private key exported as JWK → localStorage/memStore.
// Public key exported as spki (base64) → included in node announcements.

const _sigKeys = { pub: "" as string, priv: null as CryptoKey | null };
// Cache of peer signing public keys: nodeId → CryptoKey
const _peerSigKeys = new Map<string, CryptoKey>();

export async function initSigKeys(): Promise<string> {
  if (_sigKeys.pub) return _sigKeys.pub;
  if (Platform.OS !== "web") return "";
  try {
    // Try to load persisted keypair
    const storedPriv = localStorage.getItem("hex-sig-priv-jwk");
    const storedPub  = localStorage.getItem("hex-sig-pub");
    if (storedPriv && storedPub) {
      const jwk = JSON.parse(storedPriv);
      _sigKeys.priv = await crypto.subtle.importKey(
        "jwk", jwk, { name: "ECDSA", namedCurve: "P-256" }, false, ["sign"]
      );
      _sigKeys.pub = storedPub;
      return _sigKeys.pub;
    }
    ...
    _sigKeys.pub  = uint8ArrayToBase64(new Uint8Array(pubSpki));
    _sigKeys.priv = kp.privateKey;
    localStorage.setItem("hex-sig-priv-jwk", JSON.stringify(privJwk));
    localStorage.setItem("hex-sig-pub",       _sigKeys.pub);
  } catch {}
  return _sigKeys.pub;
}

// Import a peer's signing public key (spki base64) and cache it
async function _importPeerSigKey(peerId: string, pubB64: string): Promise<void> {
  if (_peerSigKeys.has(peerId) || !pubB64) return;
  try {
    const raw = base64ToUint8Array(pubB64);
    const key = await crypto.subtle.importKey(
      "spki", raw, { name: "ECDSA", namedCurve: "P-256" }, false, ["verify"]
    );
    _peerSigKeys.set(peerId, key);
  } catch {}
}

// Sign a packet — returns base64 signature or "" on failure/native
async function _signPacket(packet: object): Promise<string> {
  if (Platform.OS !== "web" || !_sigKeys.priv) return "";
  try {
    const { sig: _s, ...rest } = packet as any;
    const canonical = JSON.stringify(rest, Object.keys(rest).sort());
    const raw = await crypto.subtle.sign(
      { name: "ECDSA", hash: "SHA-256" }, _sigKeys.priv,
...
// Verify a packet — returns true if valid, or true if no key cached yet (first-seen grace)
async function _verifyPacket(packet: any, sig: string): Promise<boolean> {
  if (Platform.OS !== "web") return true;
  if (!sig) return false; // signed types must have sig
  const peerKey = _peerSigKeys.get(packet.from);
  if (!peerKey) return true; // haven't received their pubkey yet — allow through, will verify later
  try {
    const { sig: _s, ...rest } = packet as any;
    const canonical = JSON.stringify(rest, Object.keys(rest).sort());
    return await crypto.subtle.verify(
      { name: "ECDSA", hash: "SHA-256" }, peerKey,
      base64ToUint8Array(sig), new TextEncoder().encode(canonical)
    );
  } catch { return false; }
}

// Control packet types that MUST be signed and MUST verify before processing
const SIGNED_PACKET_TYPES = new Set([
  "ECDH_HELLO", "CALL_OFFER", "CALL_ANSWER", "CALL_END", "CALL_REJECT"
]);

// ─── Auto-discovery of local HexMesh server ───────────────────────────────────
// On mobile/native: scan the local subnet for a running HexMesh server.
// Tries the current WiFi gateway range (e.g. 192.168.1.x) in parallel.
// On web: not needed — URL is derived from window.location automatically.
//
// Returns the first responding server URL, or null if none found.
export async function discoverLocalServer(
  onProgress?: (found: string | null, scanned: number, total: number) => void
): Promise<string | null> {
  if (Platform.OS === "web") return null; // web uses window.location

  // Try stored IP first (fastest)
  const stored = _memStore.get("hex_server_ip");
  if (stored) {
    try {
      const r = await fetch(`http://${stored}:3001/health`, { signal: AbortSignal.timeout(1500) });
      if (r.ok) return stored;
    } catch {}
    // Stored IP dead — fall through to scan
  }

  // Determine subnet to scan from device's own IP
  // React Native doesn't expose local IP directly, so we try common home/office ranges
  const subnets = ["192.168.1", "192.168.0", "192.168.43", "10.0.0", "10.0.1", "172.16.0"];
  const port = 3001;
  const TIMEOUT_MS = 800;
  const PARALLEL = 20; // scan 20 IPs at a time

  for (const subnet of subnets) {
    const ips: string[] = [];
    for (let i = 1; i <= 254; i++) ips.push(`${subnet}.${i}`);

    for (let i = 0; i < ips.length; i += PARALLEL) {
      const batch = ips.slice(i, i + PARALLEL);
      const results = await Promise.allSettled(
        batch.map(async ip => {
          ...
          if (data.status !== "ok") throw new Error("not hexmesh");
          return ip;
        })
      );
      onProgress?.(null, i + batch.length, ips.length);
      for (const r of results) {
        if (r.status === "fulfilled") {
          const ip = r.value;
          console.log("[DISCOVERY] found HexMesh server at", ip);
          _memStore.set("hex_server_ip", ip);
          _scheduleFlush();
          onProgress?.(ip, i + batch.length, ips.length);
          return ip;
        }
      }
    }
...
type InboundTransfer = {
  ...
  assembledData?: string; // set when all chunks received but FILE_META not yet arrived
};
...
function persistInboundChunk(id: string, index: number, data: string) {
  // On web: skip localStorage for chunks — quota is only 5 MB and large files will fail silently.
  // Chunks are already held in memory via session.receivedChunks Map.
  if (Platform.OS === "web") return;
...
type MeshContextType = {
  ...
  // Mesh latency summary across all direct peers
  meshLatency: { avg: number | null; min: number | null; max: number | null; p95: number | null };
  // Encryption status per peer: e2e = SubtleCrypto AES-GCM, native-e2e = pure-JS AES-CTR, none = plaintext
  encryptionStatus: Map<string, "e2e" | "native-e2e" | "none">;
};
...
const MeshContext = createContext<MeshContextType>({
  ...
  setServerIp: () => {},
  currentServerIp: "",
  meshLatency: { avg: null, min: null, max: null, p95: null },
...
export function MeshProvider({ children }: { children: React.ReactNode }) {
  ...
  // ── Mesh-wide latency summary ──────────────────────────────────────────────
  const [meshLatency, setMeshLatency] = useState<{ avg: number|null; min: number|null; max: number|null; p95: number|null }>(
    { avg: null, min: null, max: null, p95: null }
  );

  // ── Per-peer encryption status ─────────────────────────────────────────────
  const [encryptionStatus, setEncryptionStatus] = useState<Map<string, "e2e" | "native-e2e" | "none">>(new Map());
  ...
  useEffect(() => {
    ...
    // Init ECDSA signing keypair (replaces broken HMAC)
    initSigKeys();
    if (Platform.OS !== "web") {
  ...
  const updatePeerScore = (peerId: string, patch: Partial<PeerScore>) => {
    ...
    setPeerScores(prev => {
      ...
      // Recompute mesh-wide latency summary
      const latencies = [...m.values()]
        .map(s => s.latency)
      ...
      if (latencies.length > 0) {
        ...
        setMeshLatency({ avg, min, max, p95 });
      } else {
        setMeshLatency({ avg: null, min: null, max: null, p95: null });
      }
  ...
  // dvTable: dest → { hops, via, ts }
  // Each node broadcasts its routing table to direct neighbors every 15s.
  // Neighbors accept shorter paths; stale entries (>45s) are pruned.
  const ROUTE_AD_INTERVAL_MS = 15_000;
  const ROUTE_STALE_MS       = 45_000;
  const dvTable = useRef<Map<string, { hops: number; via: string; ts: number }>>(new Map());
  const routeAdTimer = useRef<ReturnType<typeof setInterval> | null>(null);

  const _broadcastRouteAd = () => {
    // Build our view: direct peers = 1 hop, known routes from dvTable
    const allEntries: { dest: string; hops: number; via: string }[] = [];
    for (const [pid] of dataChannels.current) {
      if (dataChannels.current.get(pid)?.readyState === "open") {
        allEntries.push({ dest: pid, hops: 1, via: "" });
      }
    }
    for (const [dest, row] of dvTable.current) {
      if (!allEntries.find(e => e.dest === dest)) {
        allEntries.push({ dest, hops: row.hops, via: row.via });
      }
    }
    if (allEntries.length === 0) return;

    // SPLIT HORIZON: for each neighbor, omit routes learned via that neighbor.
    // This prevents routing loops where A tells B about a route A learned from B.
    for (const [pid, dc] of dataChannels.current) {
      if (dc.readyState !== "open") continue;
      const filtered = allEntries
        .filter(e => e.via !== pid)          // don't advertise back what we learned from pid
        .map(({ dest, hops }) => ({ dest, hops }));
      if (filtered.length === 0) continue;
      try {
        dc.send(JSON.stringify({
          type: "ROUTE_AD", from: nodeId, ts: Date.now(), routes: filtered,
        }));
      } catch {}
    }
  };

  const _handleRouteAd = (packet: any) => {
    const from = packet.from as string;
    const now = Date.now();
    let changed = false;
    for (const { dest, hops } of (packet.routes ?? []) as { dest: string; hops: number }[]) {
      if (dest === nodeId) continue;
      const newHops = hops + 1;
      const existing = dvTable.current.get(dest);
      // Accept if: no existing route, shorter path, or update from same peer
      if (!existing || newHops < existing.hops || existing.via === from) {
        dvTable.current.set(dest, { hops: newHops, via: from, ts: now });
        // Update meshRoutesRef with the logical path [nodeId, via, dest]
        const path = newHops === 1 ? [nodeId, dest] : [nodeId, from, dest];
        meshRoutesRef.current.set(dest, path);
        changed = true;
      }
    }
    // Prune stale entries
    for (const [dest, row] of dvTable.current) {
      if (now - row.ts > ROUTE_STALE_MS) {
        dvTable.current.delete(dest);
        changed = true;
      }
    }
    if (changed) setMeshRoutes(new Map(meshRoutesRef.current));
  };

  const startRouteAd = () => {
    if (routeAdTimer.current) return;
    // Small initial delay so DCs open first
    setTimeout(_broadcastRouteAd, 2000);
    routeAdTimer.current = setInterval(_broadcastRouteAd, ROUTE_AD_INTERVAL_MS);
  };

  // ─── Peer exchange (self-bootstrapping mesh) ──────────────────────────────
  // When a DC opens, we immediately send the new peer our list of known nodes.
  // They do the same. If either side sees a node they don't have a DC to, they
  // initiate one. This makes the signaling server optional after first contact.

  const _sendPeerHello = (peerId: string, dc: RTCDataChannel) => {
    // Collect all open DC peers + nodes from server list
    const knownPeers = nodesRef.current
      .filter(n => n.nodeId !== nodeId && n.nodeId !== peerId)
      .map(n => ({ nodeId: n.nodeId, alias: n.alias, publicKey: n.publicKey, sigPub: (n as any).sigPub ?? "" }));
    if (knownPeers.length === 0) return;
    try {
      dc.send(JSON.stringify({
        type: "PEER_HELLO", from: nodeId, ts: Date.now(),
        payload: { peers: knownPeers, alias, sigPub: _sigKeys.pub },
      }));
    } catch {}
  };

  const _handlePeerHello = (packet: any) => {
    const peers: { nodeId: string; alias: string; publicKey: string; sigPub: string }[] =
      packet.payload?.peers ?? [];
    peers.forEach(p => {
      if (p.nodeId === nodeId) return;
      // Cache their signing key if new
      if (p.sigPub) _importPeerSigKey(p.nodeId, p.sigPub);
      // Add to node list if unknown
      if (!nodesRef.current.find(n => n.nodeId === p.nodeId)) {
        const newNode = { nodeId: p.nodeId, alias: p.alias, publicKey: p.publicKey, lastSeen: Date.now() } as any;
        if (p.sigPub) newNode.sigPub = p.sigPub;
        setNodes(prev => [...prev, newNode]);
      }
      // Connect if no open DC yet
      if (dataChannels.current.get(p.nodeId)?.readyState !== "open") {
        console.log("[PEER_HELLO] discovered", p.alias, "— initiating DC");
        _initiateDataChannel(p.nodeId);
      }
    });
  };

  const stopRouteAd = () => {
    if (routeAdTimer.current) { clearInterval(routeAdTimer.current); routeAdTimer.current = null; }
  };
  ...
  // ─── Congestion control for relay ─────────────────────────────────────────
  // Track per-peer relay byte counters in a sliding 1-second window.
  // If a peer is pushing too much traffic through us as relay, we drop/defer.
  const DC_BACKPRESSURE_BYTES = 256 * 1024;  // pause if DC buffer > 256 KB
  const RELAY_RATE_LIMIT_BPS  = 512 * 1024;  // max 512 KB/s relay traffic per peer
  const RELAY_TOTAL_LIMIT_BPS = 2 * 1024 * 1024; // max 2 MB/s total relay load
  const _relayBytes = new Map<string, { bytes: number; windowStart: number }>();
  let   _relayTotalBytes = 0;
  let   _relayTotalWindowStart = Date.now();

  const _checkRelayCongestion = (fromPeer: string, dataLen: number): boolean => {
    const now = Date.now();
    // Per-peer window
    const peer = _relayBytes.get(fromPeer) ?? { bytes: 0, windowStart: now };
    if (now - peer.windowStart > 1000) { peer.bytes = 0; peer.windowStart = now; }
    if (peer.bytes + dataLen > RELAY_RATE_LIMIT_BPS) return true; // congested
    peer.bytes += dataLen;
    _relayBytes.set(fromPeer, peer);
    // Total relay window
    if (now - _relayTotalWindowStart > 1000) { _relayTotalBytes = 0; _relayTotalWindowStart = now; }
    if (_relayTotalBytes + dataLen > RELAY_TOTAL_LIMIT_BPS) return true;
    _relayTotalBytes += dataLen;
    return false;
  };
  ...
  // ─── Mesh-first routing ────────────────────────────────────────────────────
  // Priority: 1) direct DataChannel  2) next-hop routing table  3) flood  4) WS last-resort
  const _meshForward = (packet: any): boolean => {
    ...
    // Congestion control: rate-limit relay traffic from any single peer
    if (packet.from && packet.from !== nodeId) {
      if (_checkRelayCongestion(packet.from, serialized.length)) {
        console.warn("[RELAY] congestion drop from", packet.from, "type:", packet.type);
        return false;
      }
    }

    // 1. Direct DC to destination
    const directDC = dataChannels.current.get(packet.to);
    if (directDC?.readyState === "open") {
      if (_dcSend(directDC, serialized)) return true;
    }

    // 2. Next-hop routing table (if we have a known path)
    const knownRoute = meshRoutesRef.current.get(packet.to);
    if (knownRoute && knownRoute.length >= 2) {
      const nextHop = knownRoute[1]; // first hop after us
      if (nextHop && nextHop !== nodeId && !visited.includes(nextHop)) {
        const hopDC = dataChannels.current.get(nextHop);
        if (hopDC?.readyState === "open") {
          if (_dcSend(hopDC, serialized)) return true;
        }
      }
    }

    // 3. Flood to all open DCs not in visited path (fallback)
    let forwarded = false;
    for (const [pid, dc] of dataChannels.current) {
  ...
  const _handleECDHHello = async (packet: any) => {
    ...
    if (!peerPub) return;
    // Web: derive AES-GCM key via SubtleCrypto
    const key = await _deriveSharedKey(peerPub);
    if (key) {
      _sharedKeys.set(packet.from, key);
      setEncryptionStatus(prev => { const m = new Map(prev); m.set(packet.from, "e2e"); return m; });
    }
    // Native: derive shared secret as SHA-256(myPub + peerPub) for pure-JS encryption
    if (Platform.OS !== "web" && _ecdhKeys.pub) {
      const keys = [_ecdhKeys.pub, peerPub].sort();
      const secret = sha256Sync(new TextEncoder().encode(keys[0] + keys[1]));
      _nativeSharedSecrets.set(packet.from, secret);
      setEncryptionStatus(prev => { const m = new Map(prev); m.set(packet.from, "native-e2e"); return m; });
    }
    _sendECDHHello(packet.from); // reply if not yet sent
  };

  const _encryptText = async (peerId: string, text: string): Promise<{ text?: string; enc?: string; native?: string }> => {
    // Web: use SubtleCrypto AES-GCM (ECDH-derived key)
    if (Platform.OS === "web") {
      const key = _sharedKeys.get(peerId);
      if (!key) return { text };
      try { return { enc: await _e2eEncrypt(key, text) }; } catch { return { text }; }
    }
    // Native (Hermes): use pure-JS AES-128-CTR + HMAC
    const secret = _nativeSharedSecrets.get(peerId);
    if (!secret) return { text };
    try { return { native: _nativeEncrypt(secret, text) }; } catch { return { text }; }
  };

  const _decryptText = async (peerId: string, payload: any): Promise<string> => {
    // Web: SubtleCrypto path
    if (payload?.enc && Platform.OS === "web") {
      const key = _sharedKeys.get(peerId);
    ...
    if (payload?.native) {
      const secret = _nativeSharedSecrets.get(peerId);
      if (secret) { const p = _nativeDecrypt(secret, payload.native); if (p !== null) return p; }
      // Fallback: try web key if cross-platform message
      const key = _sharedKeys.get(peerId);
      if (key) { const p = await _e2eDecrypt(key, payload.native); if (p !== null) return p; }
    }
    return payload?.text ?? "";
  };
  ...
  const _setupDC = (dc: RTCDataChannel, peerId: string) => {
    ...
    dc.onopen = () => {
      ...
      _sendPeerHello(peerId, dc); // ← peer exchange: share known nodes
      ...
      // Re-announce active file transfers to this peer (with real metadata)
        ...
        if (!t.done && t.to === peerId) {
          dc.send(JSON.stringify({
            ...
            payload: {
              ...
              name: t.meta?.name ?? "",
              size: t.meta?.size ?? 0,
              mimeType: t.meta?.mimeType ?? "application/octet-stream",
  ...
  const startFileTransfer = async (
    ...
    const t: OutboundTransfer = {
      ...
      meta: { name: meta.name, size: meta.size, mimeType: meta.mimeType },
    };
    ...
    // Send FILE_META multiple times to ensure delivery (UDP-like DataChannel may drop it)
    const sendMeta = () => sendPacket(to, {
      id: generateId(), type: "FILE_META", from: nodeId, to, ttl: 7, visited: [],
      payload: { transferId, name: meta.name, size: meta.size, mimeType: meta.mimeType, totalChunks, sha256 },
      ts: Date.now(),
    });
    sendMeta();
    const metaRetry1 = setTimeout(sendMeta, 500);
    const metaRetry2 = setTimeout(sendMeta, 2000);
    ...
    // Send FILE_META one final time after all chunks — receiver may have missed the first sends
    clearTimeout(metaRetry1); clearTimeout(metaRetry2);
    sendMeta();
  };
  ...
  const handleFileMeta = (packet: any) => {
    ...
    if (!existing) {
      ...
      // Only update fields if the new values are non-empty (re-announce packets may carry empty name/size)
    ...
    const session = inbound.current.get(transferId)!;
    const already = session.receivedChunks.size;
    updateProgress(transferId, { transferId, direction: "in", name: session.name || name, size: session.size || size, progress: Math.round((already / totalChunks) * 100), status: "transferring" });

    // If all chunks arrived before FILE_META, finalize now
    if (session.assembledData !== undefined) {
      dropInboundSession(transferId, totalChunks);
      inbound.current.delete(transferId);
      updateProgress(transferId, { progress: 100, status: "done" });
      const msgId = generateId();
      setMessages(p => [...p, { id: msgId, from: session.from, to: nodeId, text: "", ts: Date.now(), status: "delivered", type: "file", fileInfo: { name: session.name, size: session.size, data: session.assembledData, mimeType: session.mimeType } }]);
      seenMessages.current.add(msgId);
    }
  };
  ...
  const handleFileChunk = async (packet: any) => {
    ...
    if (session.receivedChunks.size === totalChunks) {
      ...
      // If we don't have real metadata yet (name = "file", size = 0), store assembled data
      // and wait for FILE_META to arrive — it will finalize and emit the message.
      if (!session.sha256 || session.name === "file") {
        session.assembledData = assembled;
        // Request FILE_META re-send from sender
        sendPacket(session.from, { id: generateId(), type: "FILE_META_REQ", from: nodeId, to: session.from, ttl: 7, visited: [], payload: { transferId }, ts: Date.now() });
        return;
      }
      dropInboundSession(transferId, totalChunks); inbound.current.delete(transferId);
      updateProgress(transferId, { progress: 100, status: "done" });
  ...
  const reannounceActiveTransfers = () => {
    for (const [, t] of outbound.current) {
      if (!t.done) sendPacket(t.to, {
        id: generateId(), type: "FILE_META", from: nodeId, to: t.to, ttl: 7, visited: [],
        payload: {
          transferId: t.transferId,
          name: t.meta?.name ?? "",
          size: t.meta?.size ?? 0,
  ...
  // ─── Packet router ─────────────────────────────────────────────────────────
  const ROUTABLE_TYPES = new Set(["MSG","ACK","FILE_META","FILE_CHUNK","FILE_ACK","FILE_META_REQ",
    "FILE_INTEGRITY_FAIL","CALL_OFFER","CALL_ANSWER","CALL_ICE","CALL_END","CALL_REJECT","ECDH_HELLO","ROUTE_AD","PEER_HELLO"]);

  const handlePacket = async (packet: any) => {
    // Verify ECDSA signature on signed control packets.
    // Each node signs with its private key; we verify with the sender's public key
    // (cached from NODE_LIST/NODE_JOINED sigPub field). This is real authentication.
    if (SIGNED_PACKET_TYPES.has(packet.type)) {
      if (!packet.sig) {
        console.warn("[SECURITY] unsigned control packet dropped:", packet.type, "from", packet.from);
        return;
      }
      const ok = await _verifyPacket(packet, packet.sig);
      if (!ok) {
        console.warn("[SECURITY] ECDSA verification failed for", packet.type, "from", packet.from);
        return;
      }
    }
    ...
    switch (packet.type) {
      ...
      case "NODE_JOINED":
        if (packet.nodeId !== nodeId) {
          ...
          if (packet.sigPub)    _importPeerSigKey(packet.nodeId, packet.sigPub);
        }
        break;
      case "NODE_LEFT":
      ...
      case "FILE_META_REQ": {
        // Receiver asking us to resend FILE_META (arrived after chunks)
        const t = outbound.current.get(packet.payload?.transferId);
        if (t) sendPacket(packet.from, {
          id: generateId(), type: "FILE_META", from: nodeId, to: packet.from, ttl: 7, visited: [],
          payload: { transferId: t.transferId, name: t.meta.name, size: t.meta.size, mimeType: t.meta.mimeType, totalChunks: t.totalChunks, sha256: t.sha256 },
          ts: Date.now(),
        });
        break;
      }
      ...
      case "PEER_HELLO":
        // Peer exchange: learn about nodes this peer knows, connect to unknowns
        _handlePeerHello(packet);
        break;
      case "ROUTE_AD":
        // Distance-vector routing advertisement from a neighbor
        _handleRouteAd(packet);
        break;
      case "PING":
        // WS ping from server
        if (packet.from && packet.from !== nodeId) {
  ...
  const connect = () => {
    ...
    // On native: if no server URL yet, try to auto-discover one on the LAN
    const doConnect = () => {
      const url = getCurrentSignalingUrl();
      if (!url) {
        // No URL available — keep status as connecting, retry after discovery
        console.log("[CONNECT] no server URL, auto-discovery will find one...");
        scheduleReconnect();
        return;
      }
      ...
      socket.onopen = () => {
        ...
        socket.send(JSON.stringify({ type: "REGISTER", nodeId, alias, publicKey: _ecdhKeys.pub || "", sigPub: _sigKeys.pub || "" }));
    ...
    // On native with no stored IP — scan LAN automatically
    if (Platform.OS !== "web" && !_memStore.get("hex_server_ip")) {
      discoverLocalServer().then(ip => {
        if (ip) { doConnect(); }
  ...
  return (
    <MeshContext.Provider value={{
      ...
      currentServerIp: Platform.OS === "web"
        ? (window?.location?.hostname ?? "localhost")
        : (_memStore.get("hex_server_ip") ?? ""),
      meshLatency,
      encryptionStatus,
    }}>
      {children}

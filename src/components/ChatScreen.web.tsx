                  ...
                  <div className="flex items-center gap-2 min-w-[180px]">
                    ...
                    {msg.fileInfo.data && (
                      <button
                        onClick={() => {
                          try {
                            const bytes = Uint8Array.from(atob(msg.fileInfo!.data!), c => c.charCodeAt(0));
                            const blob = new Blob([bytes], { type: msg.fileInfo!.mimeType ?? "application/octet-stream" });
                            const url = URL.createObjectURL(blob);
                            const a = document.createElement("a");
                            a.href = url; a.download = msg.fileInfo!.name;
                            a.click();
                            setTimeout(() => URL.revokeObjectURL(url), 10000);
                          } catch (e) { console.error("download failed", e); }
                        }}
                        className="w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors cursor-pointer"
                      >
